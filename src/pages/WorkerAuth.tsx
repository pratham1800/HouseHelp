...
export default function WorkerAuth() {
  ...
  const { loading: authLoading, signIn, signUp, signInWithGoogle } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        // Sign up as 'worker' since this is the worker portal
        const { error } = await signUp(email, password, fullName, 'worker');
        if (error) {
          toast({
